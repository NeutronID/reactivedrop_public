...
ConVar rd_jump_jet_usages( "rd_jump_jet_usages", "0", FCVAR_CHEAT, "Set to 0 for default, -1 for infinite." );
...
{
	BaseClass::Spawn();
	if (rd_jump_jet_usages.GetInt() == -1)
		m_bInfinite = true;
	else if (rd_jump_jet_usages.GetInt() >= 1)
	{
		m_bInfinite = false;
		m_iClip1 = rd_jump_jet_usages.GetInt();
	}
	else
		m_bInfinite = false;
}
